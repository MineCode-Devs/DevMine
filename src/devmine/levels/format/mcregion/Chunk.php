...
namespace devmine\levels\format\mcregion;
...
class Chunk extends BaseFullChunk{
	...
	public function __construct($level, CompoundTag $nbt = null){
		...
		if(isset($this->nbt->solidentityEntities) and $this->nbt->solidentityEntities instanceof ListTag){
			$this->nbt->solidentityEntities->setTagType(NBT::TAG_Compound);
		}else{
			$this->nbt->solidentityEntities = new ListTag("solidentityEntities", []);
			$this->nbt->solidentityEntities->setTagType(NBT::TAG_Compound);
		}

		if(isset($this->nbt->solidentityTicks) and $this->nbt->solidentityTicks instanceof ListTag){
			$this->nbt->solidentityTicks->setTagType(NBT::TAG_Compound);
		}else{
			$this->nbt->solidentityTicks = new ListTag("solidentityTicks", []);
			$this->nbt->solidentityTicks->setTagType(NBT::TAG_Compound);
		}
		...
		parent::__construct($level, $this->nbt["xPos"], $this->nbt["zPos"], $this->nbt->Blocks->getValue(), $this->nbt->Data->getValue(), $this->nbt->SkyLight->getValue(), $this->nbt->BlockLight->getValue(), $this->nbt->BiomeColors->getValue(), $this->nbt->HeightMap->getValue(), $this->nbt->Entities->getValue(), $this->nbt->solidentityEntities->getValue(), $extraData);
	...
	public function toBinary(){
		...
		$nbt->solidentityEntities = new ListTag("solidentityEntities", $solidentities);
		$nbt->solidentityEntities->setTagType(NBT::TAG_Compound);
