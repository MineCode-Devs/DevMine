...
use devmine\inventory\blocks\NetherWart;
...
use devmine\inventory\blocks\CocoaBlock;
...
use devmine\server\events\block\BlockPlaceEvent;
use devmine\server\events\block\BlockUpdateEvent;
use devmine\server\events\level\ChunkLoadEvent;
use devmine\server\events\level\ChunkPopulateEvent;
use devmine\server\events\level\ChunkUnloadEvent;
use devmine\server\events\level\LevelSaveEvent;
use devmine\server\events\level\LevelUnloadEvent;
use devmine\server\events\level\SpawnChangeEvent;
use devmine\server\events\LevelTimings;
...
use devmine\levels\format\generic\EmptyChunkSection;
use devmine\levels\format\LevelProvider;
use devmine\levels\generator\GenerationTask;
use devmine\levels\generator\Generator;
use devmine\levels\generator\GeneratorRegisterTask;
use devmine\levels\generator\GeneratorUnregisterTask;
use devmine\levels\generator\LightPopulationTask;
use devmine\levels\generator\PopulationTask;
...
use devmine\server\epilogos\BlockepilogosStore;
...
use devmine\levels\sound\Sound;
...
use devmine\levels\weather\WeatherManager;

#include <rules/Level.h>

class Level implements ChunkManager, epilogosble{
	...
	/** @var BlockepilogosStore */
	private $blockepilogos;
	...
	public function __construct(Server $server, $name, $path, $provider){
		...
		$this->blockepilogos = new BlockepilogosStore($this);
		$this->server = $server;
	...
	/**
	 * @return BlockepilogosStore
	 */
	public function getBlockepilogos(){
		return $this->blockepilogos;
	}
	...
	public function doTick($currentTick){
		...
		$this->timings->solidentityEntityTick->startTiming();
		Timings::$ticksolidentityEntityTimer->startTiming();
		//Update solidentities that need update
		if(count($this->updatesolidentities) > 0){
			foreach($this->updatesolidentities as $id => $solidentity){
				if($solidentity->onUpdate() !== true){
					unset($this->updatesolidentities[$id]);
				}
			}
		}
		Timings::$ticksolidentityEntityTimer->stopTiming();
		$this->timings->solidentityEntityTick->stopTiming();

		$this->timings->doTicksolidentities->startTiming();
		if(($currentTick % 2) === 0) $this->tickChunks();
		$this->timings->doTicksolidentities->stopTiming();
	...
	*/
	...
	public function useBreakOn(Vector3 $vector, Item &$item = null, Player $player = null, $createParticles = false){
		...
		if($player !== null){
			...
			$drops = $target->getDrops($item); //Fixes solidentity entities being deleted before getting drops
		...
		$solidentity = $this->getsolidentity($target);
		if($solidentity !== null){
			if($solidentity instanceof InventoryHolder){
				if($solidentity instanceof Chest){
					$solidentity->unpair();
				}

				foreach($solidentity->getInventory()->getContents() as $chestItem){
	...
	public function useItemOn(Vector3 $vector, Item &$item, $face, $fx = 0.0, $fy = 0.0, $fz = 0.0, Player $player = null){
		...
		if($hand->getId() === Item::SIGN_POST or $hand->getId() === Item::WALL_SIGN){
			...
			$nbt = new CompoundTag("", [
				"id" => new StringTag("id", solidentity::SIGN),
			...
			solidentity::createsolidentity("Sign", $this->getChunk($block->x >> 4, $block->z >> 4), $nbt);
		}
	...
	/**
	 * Returns a list of the solidentity entities in this level
	 *
	 * @return solidentity[]
	 */
	 ...
	 * @return solidentity
	 */
	public function getsolidentityById($solidentityId){
		return isset($this->solidentities[$solidentityId]) ? $this->solidentities[$solidentityId] : null;
	}
	...
	/**
	 * Returns the solidentity in a position, or null if not found
	 *
	 * @param Vector3 $pos
	 *
	 * @return solidentity
	 */
	public function getsolidentity(Vector3 $pos){
		$chunk = $this->getChunk($pos->x >> 4, $pos->z >> 4, false);

		if($chunk !== null){
			return $chunk->getsolidentity($pos->x & 0x0f, $pos->y & 0xff, $pos->z & 0x0f);
		}
	...
	/**
	 * Gives a list of the solidentity entities on a given chunk
	 *
	 * @param int $X
	 * @param int $Z
	 *
	 * @return solidentity[]
	 */
	public function getChunksolidentities($X, $Z){
		return ($chunk = $this->getChunk($X, $Z)) !== null ? $chunk->getsolidentities() : [];
	}
	...
	public function setChunk(int $chunkX, int $chunkZ, FullChunk $chunk = null, bool $unload = true){
		...
		if($unload and $oldChunk !== null){
			...
			$oldsolidentities = $oldChunk !== null ? $oldChunk->getsolidentities() : [];
			...
			foreach($oldsolidentities as $solidentity){
				$chunk->addsolidentity($solidentity);
				$solidentity->chunk = $chunk;
			}
		}
	...
	public function addsolidentity(solidentity $solidentity){
		if($solidentity->getLevel() !== $this){
			throw new LevelException("Invalid solidentity level");
		}
		$this->solidentities[$solidentity->getId()] = $solidentity;
		$this->clearChunkCache($solidentity->getX() >> 4, $solidentity->getZ() >> 4);
	}
	...
	public function removesolidentity(solidentity $solidentity){
		...
		unset($this->updatesolidentities[$solidentity->getId()]);
		$this->clearChunkCache($solidentity->getX() >> 4, $solidentity->getZ() >> 4);
	}
		...
		try{
			if($chunk !== null){
				if($trySave and $this->getAutoSave()){
					...
					if($chunk->hasChanged() or count($chunk->getsolidentities()) > 0 or $entities > 0){
	...
	public function setepilogos($epilogosKey, epilogosValue $epilogosValue){
		$this->server->getLevelepilogos()->setepilogos($this, $epilogosKey, $epilogosValue);
	}

	public function getepilogos($epilogosKey){
		return $this->server->getLevelepilogos()->getepilogos($this, $epilogosKey);
	}

	public function hasepilogos($epilogosKey){
		return $this->server->getLevelepilogos()->hasepilogos($this, $epilogosKey);
	}

	public function removeepilogos($epilogosKey, Plugin $plugin){
		$this->server->getLevelepilogos()->removeepilogos($this, $epilogosKey, $plugin);
	}
