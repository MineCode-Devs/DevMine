...
use devmine\creatures\entities\ai\AIHolder;
use devmine\server\events\HandlerList;
use devmine\server\events\level\LevelInitEvent;
use devmine\server\events\level\LevelLoadEvent;
use devmine\server\events\server\QueryRegenerateEvent;
use devmine\server\events\server\ServerCommandEvent;
...
use devmine\levels\format\anvil\Anvil;
use devmine\levels\format\leveldb\LevelDB;
use devmine\levels\format\LevelProviderManager;
use devmine\levels\format\mcregion\McRegion;
use devmine\levels\generator\biome\Biome;
use devmine\levels\generator\Flat;
use devmine\levels\generator\Void;
use devmine\levels\generator\Generator;
use devmine\levels\generator\hell\Nether;
use devmine\levels\generator\normal\Normal;
use devmine\levels\generator\normal\Normal2;
use devmine\levels\Level;
use devmine\server\epilogos\EntityepilogosStore;
use devmine\server\epilogos\LevelepilogosStore;
use devmine\server\epilogos\PlayerepilogosStore;
...
use devmine\server\network\upnp\UPnP;
use devmine\server\permissions\BanList;
use devmine\server\permissions\DefaultPermissions;
use devmine\pluginfeatures\FolderPluginLoader;
use devmine\pluginfeatures\PharPluginLoader;
use devmine\pluginfeatures\Plugin;
use devmine\pluginfeatures\PluginLoadOrder;
use devmine\pluginfeatures\PluginManager;
use devmine\pluginfeatures\ScriptPluginLoader;
...
use devmine\server\updater\AutoUpdater;
...
class Server{
	...
	/** @var EntityepilogosStore */
	private $entityepilogos;

	/** @var PlayerepilogosStore */
	private $playerepilogos;

	/** @var LevelepilogosStore */
	private $levelepilogos;
	...
	public function getVersion(){
		return \devmine\MINECRAFT_VERSION;
	}
	...
	public function getiTXApiVersion(){
		return \devmine\devmine_API_VERSION;
	}
	...
	/**
	 * @return EntityepilogosStore
	 */
	public function getEntityepilogos(){
		return $this->entityepilogos;
	}

	/**
	 * @return PlayerepilogosStore
	 */
	public function getPlayerepilogos(){
		return $this->playerepilogos;
	}

	/**
	 * @return LevelepilogosStore
	 */
	public function getLevelepilogos(){
		return $this->levelepilogos;
	}
	...
	public function loadLevel($name){
		...
		//if(file_exists($path . "solidentityEntities.yml")){
		//	@rename($path . "solidentityEntities.yml", $path . "solidentities.yml");
		//}
	 ...
	 * @param string $name
	 * @param int    $seed
	 * @param string $generator Class name that extends devmine\levels\generator\Noise
	 * @param array  $options
	 *
	 * @return bool
	 */
	public function generateLevel($name, $seed = null, $generator = null, $options = []){
		...
		try{
			$path = $this->getDataPath() . "worlds/" . $name . "/";
			/** @var \devmine\levels\format\LevelProvider $provider */
	...
	public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath, $defaultLang = "unknown"){
		...
		try{
			...
			$this->aboutstring = "\n
		   §5devmine-iTX §3devmine §fis a fork of devmine-MP, made by §5iTX Technologies LLC§f.
		   §fVersion: §6" . $this->getdevmineVersion() . "
		   §fTarget client Version: §d" . \devmine\MINECRAFT_VERSION . "
		   §fLatest source code is available at https://github.com/iTXTech/devmine
		   §fDonate link: http://pl.zxda.net/plugins/203.html
		\n";

			$this->about();

			$this->logger->info("Loading devmine.yml...");
			if(!file_exists($this->dataPath . "devmine.yml")){
				$content = file_get_contents($this->filePath . "src/devmine/resources/devmine.yml");
				if($version->isDev()){
			...
			if(file_exists($this->filePath . "src/devmine/resources/devmine_$lang.yml")){
				$content = file_get_contents($file = $this->filePath . "src/devmine/resources/devmine_$lang.yml");
			}else{
				$content = file_get_contents($file = $this->filePath . "src/devmine/resources/devmine_eng.yml");
			}
			...
			$this->logger->info($this->getLanguage()->translateString("devmine.server.start", [TextFormat::AQUA . $this->getVersion(). TextFormat::WHITE]));
			...
			$this->entityepilogos = new EntityepilogosStore();
			$this->playerepilogos = new PlayerepilogosStore();
			$this->levelepilogos = new LevelepilogosStore();
			...
			$this->logger->info($this->getLanguage()->translateString("devmine.server.networkStart", [$this->getIp() === "" ? "*" : $this->getIp(), $this->getPort()]));
			...
			$this->logger->info($this->getLanguage()->translateString("devmine.server.info", [
				$this->getName(),
				$this->getdevmineVersion(),
				$this->getCodename(),
				$this->getApiVersion()
			]));
			$this->logger->info($this->getLanguage()->translateString("devmine.server.license", [$this->getName()]));
			...
			$this->updater = new AutoUpdater($this, $this->getProperty("auto-updater.host", "www.devmine.net"));
	...
	public function start(){
		...
		if(!file_exists($this->getPluginPath() . DIRECTORY_SEPARATOR . "devmine")){
			@mkdir($this->getPluginPath() . DIRECTORY_SEPARATOR . "devmine");
		}
	...
	public function crashDump(){
		...
		if($this->getProperty("auto-report.enabled", true) !== false){
			...
			if($report){
				$reply = Utils::postURL("http://" . $this->getProperty("auto-report.host", "crash.devmine.net") . "/submit/api", [
					"report" => "yes",
					"name" => $this->getName() . " " . $this->getdevmineVersion(),
					"email" => "crash@devmine.net",
	...
	private function registersolidentities(){
		solidentity::registersolidentity(BrewingStand::class);
		solidentity::registersolidentity(Chest::class);
		solidentity::registersolidentity(Furnace::class);
		solidentity::registersolidentity(Sign::class);
		solidentity::registersolidentity(EnchantTable::class);
		solidentity::registersolidentity(FlowerPot::class);
		solidentity::registersolidentity(Skull::class);
		solidentity::registersolidentity(MobSpawner::class);
		solidentity::registersolidentity(ItemFrame::class);
		solidentity::registersolidentity(Dispenser::class);
		solidentity::registersolidentity(Dropper::class);
		solidentity::registersolidentity(DLDetector::class);
		solidentity::registersolidentity(Cauldron::class);
	}
}
