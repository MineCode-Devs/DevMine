...
namespace devmine\server\updater;
...
class AutoUpdater{
	...
	public function showConsoleUpdate(){
		...
		$logger->warning("----- devmine-MP Auto Updater -----");
		$logger->warning("Your version of devmine-MP is out of date. Version " . $newVersion->get(false) . " (build #" . $newVersion->getBuild() . ") was released on " . date("D M j h:i:s Y", $this->updateInfo["date"]));
	...
	public function showPlayerUpdate(Player $player){
		$player->sendMessage(TextFormat::DARK_PURPLE . "The version of devmine-MP that this server is running is out of date. Please consider updating to the latest version.");
	...
	protected function showChannelSuggestionStable(){
		$logger = $this->server->getLogger();
		$logger->info("----- devmine-MP Auto Updater -----");
		...
		$logger->info("If you would like to be kept informed about new Stable builds only, it is recommended that you change 'preferred-channel' in your devmine.yml to 'stable'.");
		$logger->info("----- -------------------------- -----");
	}
	...
	protected function showChannelSuggestionBeta(){
		...
		$logger->info("If you would like to be kept informed about new Beta or Development builds, it is recommended that you change 'preferred-channel' in your devmine.yml to 'beta' or 'development'.");
		$logger->info("----- -------------------------- -----");
	}
	...
	protected function checkUpdate(){
		...
		$currentVersion = new VersionString($this->server->getdevmineVersion());
		$newVersion = new VersionString($this->updateInfo["version"]);
